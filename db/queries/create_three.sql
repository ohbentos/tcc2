...
DROP VIEW IF EXISTS graph_view_arr CASCADE;
CREATE OR REPLACE VIEW graph_view_arr AS
SELECT
  g.*,
...
FROM
  ...
  CROSS JOIN LATERAL (
    ...
    FROM
      vertices v
    WHERE
      v.graph_id = g.graph_id
  ) v;

DROP VIEW IF EXISTS graph_view_json CASCADE;
CREATE OR REPLACE VIEW graph_view_json AS
SELECT
  g.*,
  e.edges_info, -- all edges
  v.vertices_info -- all vertices
...
DROP VIEW IF EXISTS all_view_arr;
CREATE OR REPLACE VIEW all_view_arr AS
SELECT
  g.*
FROM
  graph_view_arr g;

DROP VIEW IF EXISTS all_view_json;
CREATE OR REPLACE VIEW all_view_json AS
SELECT
  g.*
FROM
  graph_view_json g;
...
SELECT
  ...
  jsonb_agg (
    json_build_object (
      'id', id, 'vertice1', vertice1, 'vertice2', vertice2, 'p1', p1, 'p2', p2, 'p3', p3, 'p4', p4, 'p5', p5, 'p6', p6, 'p7', p7, 'p8', p8, 'p9', p9, 'p10', p10
    )
  ) as edges
FROM
  edges
...
SELECT
  ...
  jsonb_agg (
    json_build_object (
      'id', id, 'p1', p1, 'p2', p2, 'p3', p3, 'p4', p4, 'p5', p5, 'p6', p6, 'p7', p7, 'p8', p8, 'p9', p9, 'p10', p10
    )
  ) as vertices
FROM
  vertices
...
JOIN (
  SELECT
    ...
    bool_and(p1 > 10 AND p2 > 10) AS edge_filter
  FROM
    edges
  GROUP BY
    graph_id
...
JOIN (
  SELECT
    ...
    bool_and(p1 > 5) AS vertice_filter -- add vertice filter
  FROM
    edges
  GROUP BY
    graph_id
