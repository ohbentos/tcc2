...
SELECT
  g.graph_id, (unnest(g.edges)).vertices AS vertices
FROM
  graphs g;
...
FROM 
  graphs g,
  unnest(g.edges) as ed,
  unnest(ed.vertices) as vertice;

DROP VIEW IF EXISTS get_edge_view;
CREATE OR REPLACE VIEW get_edge_view AS 
SELECT
  g.graph_id,
  edge.p1, edge.p2, edge.p3, edge.p4, edge.p5, edge.p6, edge.p7, edge.p8, edge.p9, edge.p10
FROM
  graphs g
CROSS JOIN LATERAL 
  unnest(g.edges) as edge;
...
-- filter on ONE edge and vertice property
...
-- filter on ALL vertices property
...
-- filter on ALL edge and vertice property
